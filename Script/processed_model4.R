...
for (k in 1:10) {
  ...
  formula<-  as.formula('Log10_NoV_norm ~ -1 + Intercept + lockdown_step3 + lockdown_step4 + lockdown_planB +
    ...
    scale_rain_rolling_7day + scale_temp_rolling_7day +
                        ...
                        f(spatial.field, model=spde, group=spatial.field.group, control.group=list(model="ar1", hyper=rho_hyper))')
...
write.csv(metrics, "outputs/cv/cv_model4_processed_metrics_full.csv")
write.csv(predictions, "outputs/cv/cv_model4_processed_predictions.csv")
