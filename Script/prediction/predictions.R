...
stack.train <- inla.stack(
  data = list(Log10_NoV_norm = nov$Log10_NoV_norm), 
  A = list(A.train, 1), 
  effects = list(
...
# formula
formula<-  as.formula('Log10_NoV_norm ~ -1 + Intercept + lockdown_step3 + lockdown_step4 + lockdown_planB + lockdown_lifting +
