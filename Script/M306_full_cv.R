...
# temperature
temp<- read.csv("data/covariates/stw/daily_temp_stw.csv")

# rainfall
rain<- read.csv("data/covariates/stw/daily_rain_stw.csv")

# BAME, IMD
stw_cov<- read.csv("data/covariates/stw/stw_covariates.csv")

# Schools 
school<- read.csv("data/covariates/stw/school_stw.csv")

# quintile school


# Carehomes
carehome<- read.csv("data/covariates/stw/carehome_stw.csv")

# population mobility
mob<- read.csv("data/covariates/stw/stw_mobility.csv")
...
# land cover
land<- read.csv("data/covariates/stw/land_cover_cat_stw.csv")

# population density

pop_den<- read.csv("data/covariates/stw/pop_den_stw.csv")

## Nov data
nov_df<- read.csv("data/nov/nov_site_week_rect.csv")

nov_df<- nov_df[complete.cases(nov_df$avg_Log10_NoV), ]
nov_df$date<- as.Date(nov_df$collection_date)
...
nov_df<- nov_df %>%
  left_join(stw_cov %>%
              dplyr::select(site_code, imd, bame),
            by= "site_code") %>%
  left_join(school %>%
              dplyr::select(site_code, school_den),
            by= "site_code") %>%
  left_join(carehome %>%
              dplyr::select(site_code, carehome_den),
            by= "site_code") %>%
  left_join(land %>%
              dplyr::select(site_code, prop_agri, prop_urb),
            by= "site_code") %>%
  left_join(pop_den %>%
              dplyr::select(site_code, pop_den),
            by= "site_code") 
...
nov_df <- nov_df %>%
  ...
  left_join(temp %>%
              mutate(date = as.Date(date)) %>%
              dplyr::select(site_code, date, temp_cat),
            by = c("site_code", "date")) %>%
  left_join(rain %>%
              mutate(date = as.Date(date)) %>%
              dplyr::select(site_code, date, rain_cat),
            by = c("site_code", "date")) %>%
  left_join(mob %>%
              mutate(date = as.Date(date)) %>%
              dplyr::select(site_code, date, mob_7day),
            by = c("site_code", "date"))
...
# convert all dummy variables to numerical
nov_df <- nov_df %>%
  mutate(across(starts_with("temp_"), as.numeric),  # Convert all temp_ columns to numeric
         across(starts_with("rain_"), as.numeric))

nov_df$pop_den<- log(nov_df$pop_den)
## scale

covariates_to_scale <-  c("school_den", "carehome_den", "imd", "bame", "mob_7day", "prop_agri", "prop_urb", "pop_den")
...
write.csv(metrics, "outputs/cv/cv_m306_metrics_full.csv")
write.csv(predictions, "outputs/cv/cv_m306_predictions.csv")
