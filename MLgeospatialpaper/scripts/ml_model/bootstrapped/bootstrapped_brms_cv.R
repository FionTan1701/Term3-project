...
run_fold_brms <- function(k) {
  tryCatch({
    ...
    coverage <- as.numeric(test$avg_Log10_NoV>= q0.025 & test$avg_Log10_NoV <= q0.975)
    
    results <- tibble(
      row_id = test$row_id,
      fold = k,
      truth = test$avg_Log10_NoV,
      BRMS_pred  = brms_mean,
      q0.025 = q0.025,
      q0.975 = q0.975,
      coverage = coverage
...
# Save Output to .csv
readr::write_csv(data_oos, "../../output/brms_final_predictions_with_folds.csv")
readr::write_csv(final_metrics, "../../output/brms_final_metrics_by_fold.csv")
