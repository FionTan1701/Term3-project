...
dir.create("../../output/Figures/ensemble_cv/RF/", recursive = TRUE, showWarnings = FALSE)
dir.create("../../output/ensemble_cv/RF/", recursive = TRUE, showWarnings = FALSE)
...
run_fold <- function(k) {
  ...
  meta_fit <- lm(avg_Log10_NoV ~ rf1_pred + rf2_pred + rf3_pred, data = meta_train)
...
readr::write_csv(ensemble_metrics_by_fold, "../../output/ensemble_cv/RF/optimised_ensemble_rf_metrics_by_fold.csv")
...
readr::write_csv(overall_ensemble_metrics, "../../output/ensemble_cv/RF/optimised_ensemble_rf_metrics_overall.csv")
...
# Save to0 .csv
readr::write_csv(data_oos, "../../output/ensemble_cv/RF/data_oos_optimised.csv")
