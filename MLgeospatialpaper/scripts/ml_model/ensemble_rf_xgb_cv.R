...
dir.create("../../output/Figures/ensemble_cv/RF_XGB/", recursive = TRUE, showWarnings = FALSE)
dir.create("../../output/ensemble_cv/RF_XGB/", recursive = TRUE, showWarnings = FALSE)
...
run_fold <- function(k) {
  ...
  set.seed(100 + k) 
  rf_fit <- randomForest(
    avg_Log10_NoV ~ ., data = train[, c("avg_Log10_NoV", covariates)],
    ntree = 750, mtry = 5
  )
  rf_pred_train <- predict(rf_fit, newdata = train[, covariates])
  rf_pred_test  <- predict(rf_fit, newdata = test[, covariates])
  ...
  meta_fit <- lm(avg_Log10_NoV ~ rf_pred + xgb_pred, data = meta_train)
...
readr::write_csv(ensemble_metrics_by_fold, "../../output/ensemble_cv/RF_XGB/optimised_ensemble_rf_xgb_metrics_by_fold.csv")
...
readr::write_csv(overall_ensemble_metrics, "../../output/ensemble_cv/RF_XGB/optimised_ensemble_rf_xgb_metrics_overall.csv")
...
# Save to .csv
readr::write_csv(data_oos, "../../output/ensemble_cv/RF_XGB/data_oos_optimised.csv")
